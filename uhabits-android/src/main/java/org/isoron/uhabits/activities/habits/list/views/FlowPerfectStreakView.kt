...
class FlowPerfectStreakView(
    ...
    private val titleView = TextView(context).apply {
        ...
        text = resources.getString(R.string.flow_perfect_streak)
    }
    ...
    init {
        ...
        repeat(7) {
            ...
            val dayMark = ImageView(context).apply {
                ...
                val size = dim(R.dimen.flow_perfect_day_size).toInt()
        ...
        update(HabitCardListCache.PerfectStreakSummary(0, List(7) { false }))
    }

    fun update(summary: HabitCardListCache.PerfectStreakSummary) {
        ...
        val cardColors = if (summary.todayComplete) {
            intArrayOf(accent, gradientEnd)
        } else {
            intArrayOf(inactiveStart, inactiveEnd)
        }
        background = GradientDrawable(GradientDrawable.Orientation.TL_BR, cardColors).apply {
            cornerRadius = dim(R.dimen.flow_card_radius)
        }
        titleView.setTextColor(Color.WHITE)
        streakView.setTextColor(Color.WHITE)
        messageView.setTextColor(Color.WHITE)
        streakView.text = when (summary.currentStreak) {
        ...
        val today = getToday()
        val recentCompletions = summary.lastSevenDays.mapIndexed { index, complete ->
            today.minus(6 - index) to complete
        }.toMap()
        val weekStart = today.startOfWeek(preferences.firstWeekday)
